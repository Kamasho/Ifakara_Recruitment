@extends('user.userhomepage')

@section('content')
<div class="row justify-content-center">
    <div class="col-lg-12 col-md-12 col-sm-12">
        <div class="card shadow">
            <div class="card-header">
              <h2 style="float:left ; font-family: 'Times New Roman', Times, serif">Saved Academic Qualifications</h2>
              <a style="float: right; margin-top: 5px; border:none; font-size:25px; color:black;" href="{{ route('add_academicqualifications') }}"><i class="bi bi-plus-circle"></i></a>
            </div>
            <div class="card-body">
                <table class="table table-striped">
                    <thead>
                      <tr>
                        <th>Level</th>
                        <th>Name</th>
                        <th>Institution</th>
                        <th>Attachiment</th>
                        <th>Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>    
                      </tr>
                    </tbody>
                  </table>
            </div>
        </div>
        <div class="card shadow mt-5">
            <div class="card-header">
              <h2 style="font-family: 'Times New Roman', Times, serif">Saved Academic Transcripts</h2>
            </div>
            <div class="card-body">
                <table class="table table-striped">
                    <thead>
                      <tr>
                        <th>Level</th>
                        <th>Programme</th>
                        <th>Attachiment</th>
                        <th>Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>    
                      </tr>
                    </tbody>
                  </table>
            </div>
        </div>
        <div class="card shadow mt-5">
            <div class="card-header">
              <h2 style="font-family: 'Times New Roman', Times, serif">Verification Certificate</h2>
            </div>
            <div class="card-body">
                <table class="table table-striped">
                    <thead>
                      <tr>
                        <th>Level</th>
                        <th>Programme</th>
                        <th>Attachiment</th>
                        <th>Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr>
                        <td></td>
                        <td></td>
                        <td></td>
                        <td></td>    
                      </tr>
                    </tbody>
                  </table>
            </div>
        </div>
    </div>
</div>
@endsection